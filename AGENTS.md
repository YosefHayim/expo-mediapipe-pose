...
An Expo native pose-camera library using Google's MediaPipe Tasks SDK. Read [README.md](README.md) for installation/scope and [docs/api.md](docs/api.md) before changing public behavior. This file is the canonical agent instruction source.

## Ownership

- `src/components/`: camera composition and SVG presentation.
- `src/hooks/`: React lifecycle and pose-feedback hooks.
- `src/pose/`: pure landmark lookup, projection, styling and rule evaluation.
...
- `ios/`: Swift capture, detector ownership and Expo registration.
- `android/src/main/java/expo/modules/mediapipepose/`: Kotlin equivalents.
- `example/`: one standalone Expo app exercising the public API.
...
Test observable behavior: coordinate transforms, empty/uncertain detections, rule timing/transitions, updated callbacks and teardown. Do not substitute source-text assertions for native compilation or camera tests. Build success does not prove physical-camera alignment or performance. Report unavailable device checks explicitly.
